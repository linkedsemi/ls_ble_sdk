...
XIP_BANNED __attribute__((naked)) void do_spi_flash_erase_svcall(uint32_t offset,uint8_t opcode)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_erase,flash_swint_set);
}

XIP_BANNED __attribute__((naked)) void do_spi_flash_read_svcall(void *param)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_read,flash_swint_set);
}

XIP_BANNED __attribute__((naked)) void do_spi_flash_read_reg_svcall(void *param)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_read_reg,flash_swint_set);
}

XIP_BANNED __attribute__((naked)) void do_spi_flash_chip_erase_svcall(void)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_chip_erase,flash_swint_set);

}

XIP_BANNED __attribute__((naked)) void do_spi_flash_erase_security_area_svcall(uint8_t idx)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_erase_security_area,flash_swint_set);

}

XIP_BANNED __attribute__((naked)) void do_spi_flash_program_security_area_svcall(uint8_t idx, uint16_t addr, uint8_t * data, uint16_t length)
{
    SWINT_FUNC_CALL_INLINE_ASM(do_spi_flash_program_security_area,flash_swint_set);

}

XIP_BANNED __attribute__((naked)) void do_spi_flash_read_security_area_svcall(uint8_t idx, uint16_t addr, uint8_t * data, uint16_t length)
{
